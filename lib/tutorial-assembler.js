...
function injectAnnotations(code, focus, marks) {
  ...
  if (focus) {
    const { startLine, endLine } = findLineRange(code, focus.find)
    lines.splice(startLine - 1 + offset.value, 0, "  // !focus(start)")
    offset.value++
    lines.splice(endLine + offset.value, 0, "  // !focus(end)")
    offset.value++
  }
  ...
  if (marks && marks.length > 0) {
    for (const mark of marks) {
      ...
      const idx = lineNum - 1 + offset.value
