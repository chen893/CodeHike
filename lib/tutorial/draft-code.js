...
function getUsablePatchFile(value) {
  if (typeof value !== "string") return null
  const trimmed = value.trim()
  if (!trimmed) return null
  const normalized = trimmed.toLowerCase()
  if (normalized === "undefined" || normalized === "null" || normalized === "auto" || trimmed === "自动") {
    return null
  }
  return trimmed
}

function resolveKnownFile(files, candidate) {
  if (!candidate) return null
  if (candidate in files) return candidate
  const lowerTarget = candidate.toLowerCase()
  return Object.keys(files).find(f => f.toLowerCase() === lowerTarget) ?? null
}

function resolvePrimaryFile(files, primaryFile) {
  return resolveKnownFile(files, primaryFile) || Object.keys(files)[0]
}

function inferPatchTargetFile(files, patch, primaryFile) {
  const explicitFile = getUsablePatchFile(patch.file)
  if (explicitFile) {
    return {
      targetFile: resolveKnownFile(files, explicitFile),
      explicitFile,
      explicit: true,
    }
  }

  const primary = resolvePrimaryFile(files, primaryFile)
  if (patch.find && countOccurrences(files[primary], patch.find) === 1) {
    return { targetFile: primary, explicitFile: null, explicit: false }
  }

  const matchingFiles = Object.entries(files)
    .filter(([, code]) => patch.find && countOccurrences(code, patch.find) > 0)
    .map(([file]) => file)

  if (matchingFiles.length === 1) {
    return { targetFile: matchingFiles[0], explicitFile: null, explicit: false }
  }

  if (matchingFiles.length > 1) {
    throw new Error(
      `Patch 未指定目标文件且 find 文本在多个文件中匹配 (匹配: ${matchingFiles.join(', ')})`
    )
  }
...
export function applyContentPatches(codeOrFiles, patches, primaryFile) {
  ...
  for (const patch of patches) {
    const { targetFile, explicitFile, explicit } = inferPatchTargetFile(result, patch, primaryFile)
    if (!targetFile) {
      const label = explicit ? explicitFile : patch.file
      throw new Error(`Patch 目标文件 "${label}" 不存在于当前文件集中 (可用: ${knownFiles.join(', ')})`)
    }
