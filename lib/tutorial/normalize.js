...
function getUsableFileName(value) {
...
function resolveExistingFileName(candidate, entries) {
  const usable = getUsableFileName(candidate);
  if (!usable) return null;
  if (entries.includes(usable)) return usable;
  return entries.find((entry) => entry.toLowerCase() === usable.toLowerCase()) ?? null;
}
...
export function normalizeBaseCode(baseCode, meta) {
  if (typeof baseCode === 'string') {
    ...
    const fileName = getUsableFileName(meta?.fileName) || 'main.js';
  ...
  const primaryFile = resolveExistingFileName(meta?.fileName, entries) || entries[0];
  ...
  return {
    ...
    fileName:
      typeof baseCode === 'string'
        ? getUsableFileName(meta.fileName) || primaryFile
        : resolveExistingFileName(meta.fileName, Object.keys(baseCode)) || primaryFile,
  };
}
