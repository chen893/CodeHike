...
function clampRange(start, end, lineCount) {
  ...
  const e = Math.max(s, Math.min(end, lineCount))
...
function resolveFocusRange(code, focus) {
  ...
  return clampRange(start, end, lineCount)
}
...
export async function buildTutorialSteps(data) {
  ...
  // Highlight baseCode for intro display (before any patches)
  const baseHighlightedFiles = {}
  for (const fileName of Object.keys(initialFiles)) {
    const fileLang = guessLangFromFileName(fileName)
    const baseHighlighted = await highlight(
      { lang: fileLang, value: initialFiles[fileName], meta: "" },
      githubDark
    )
    baseHighlightedFiles[fileName] = { ...baseHighlighted, value: baseHighlighted.code }
  }
  const baseStep = {
    highlighted: baseHighlightedFiles[primaryFile],
    highlightedFiles: baseHighlightedFiles,
