          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {tags.map((tag) => (
              <Link key={tag.id} href={`/explore?tag=${tag.slug}`}>
                <Card className="group border-slate-200/60 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg">
                  <CardContent className="flex items-center justify-between p-5">
                    ...
                    <span className="text-slate-300 group-hover:text-cyan-400 transition-colors text-xl">
                      →
                    </span>
                  </CardContent>
                </Card>
